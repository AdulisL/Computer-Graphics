...
void Spline::Subdiv(ControlCurve *control, Curve *curve, int subdivLevel)
{
    ...
    {
        n = curve->size();
        vec2s c_p = vector<vec2>(2 * n, vec2(0.0)); // c_p = c'

        for (int k = 0; k < n - 1; k++)
        {
            // 1. c′(2k+1) = 1/2[c(k) + c(k+1)], for k=0,...,n−1
            c_p[2 * k + 1].x = 1.0/2.0 * (curve->P[k].x + curve->P[k + 1].x);
            c_p[2 * k + 1].y = 1.0/2.0 * (curve->P[k].y + curve->P[k + 1].y);

        }
        for (int k = 1; k < n-1; k++)
        {
            // 2. c′(2k) = 1/4[c'(2k-1) + 2c(k) + c'(2k+1)), for k=1,...,n−1
            c_p[2 * k].x = 1.0/4.0*(c_p[2*k - 1].x + 2*curve->P[2*k].x 
                            + c_p[2*k+1].x);
            c_p[2 * k].y = 1.0/4.0*(c_p[2*k - 1].y + 2*curve->P[2*k].y 
                            + c_p[2*k+1].y);
        }
